...
func _enter_tree() -> void:
	if Engine.is_editor_hint():
		# Only try to connect to editor signals if we're actually in the editor
		# and the EditorInterface is available
		var editor = Engine.get_singleton("EditorInterface")
		if editor != null and editor.get_selection() != null:
			if not editor.get_selection().is_connected("selection_changed", _on_selection_changed):
				editor.get_selection().connect("selection_changed", _on_selection_changed)
...
func _draw():
	if Engine.is_editor_hint():
		# Remove debug print that's causing console spam
		var c
		if _selected:
