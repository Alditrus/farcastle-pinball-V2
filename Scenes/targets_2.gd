...
func check_all_targets_down():
	if targets_down_count >= total_targets:
		...
		if missions_node:
			missions_node.record_collision(missions_node.CollisionType.TARGET_SET2)
			
			# Check if more TARGET_SET2 completions are needed and reset lights if so
			reset_lights_if_more_completions_needed(missions_node, missions_node.CollisionType.TARGET_SET2)
		
		# Turn off the target lights for this specific set
		var lights_parent = get_node_or_null("/root/Table/Lights")
		if lights_parent:
			var light_name = "target_lights_2"
